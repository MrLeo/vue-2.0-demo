...
export const setRoadList = ({commit, state}) => {
  const info = {
    ...
    jsh: state.base.indexSearch.huxing || '',
...
export const setMapList = ({commit, state}) => {
  ...
  return api.getMap(info).then(res => {
    if (res.returnCode == '00') {
      console.warn('[Leo]getMap no response => ', res)
      let topData = res.list.filter(function (item) {
        return item.renzheng == 1;
      });
      res.list = res.list.concat(topData);
